...
public class AS400JDBCConnectionPoolDataSourceBeanInfoTestcase extends Testcase {
   ...
   **/
  public AS400JDBCConnectionPoolDataSourceBeanInfoTestcase(AS400 systemObject, Vector<String> variationsToRun,
  ...
  private boolean verifyFeatureDescriptor(FeatureDescriptor fd, String dspName, String shortDesc, boolean exp,
      boolean hid) {
  ...
  private boolean verifyFeatureDescriptor(FeatureDescriptor fd, String dspName, String shortDesc, boolean exp,
      boolean hid, StringBuffer sb) {
    boolean passed = true;
    if (!fd.getDisplayName().equals(dspName)
        && !(dspName.equals("serverTraceCategories") && fd.getDisplayName().equals("serverTrace"))) // Note:
    ...
    if (!fd.getShortDescription().equals(shortDesc)) {
      sb.append(
          "Wrong short description for '" + dspName + "' : " + fd.getShortDescription() + " sb " + shortDesc + "\n");
      passed = false;
    }
    if (fd.isExpert() != exp) {
  ...
  /**
   * Verify the constructor, AS400JDBCConnectionPoolDataSourceBeanInfo(). <br>
   * Expected results:
   * <ul>
  ...
  /**
   * Verify AS400JDBCConnectionPoolDataSourceBeanInfo::getBeanDescriptor(). <br>
   * Expected results:
   * <ul>
  ...
  public void Var002() {
    try {
      ...
      if (!bd.getBeanClass().getName().equals("com.ibm.as400.access.AS400JDBCConnectionPoolDataSource")
          || bd.getCustomizerClass() != null) {
        failed("getBeanDescriptor");
        return;
      }
      if (!verifyFeatureDescriptor(bd, "AS400JDBCConnectionPoolDataSource", "AS400JDBCConnectionPoolDataSource", false,
          false)) {
  ...
  /**
   * Verify AS400JDBCConnectionPoolDataSourceBeanInfo::getDefaultEventIndex() is
   * returned from getAdditionalBeanInfo(). <br>
   * Expected results:
   * <ul>
  ...
  /**
   * Verify AS400JDBCConnectionPoolDataSource::getAdditionBeanInfo(). Expected
   * results:
   * <UL>
  ...
  /**
   * Verify AS400JDBCConnectionPoolDataSource::getBeanDescriptor() returned from
   * getAdditionalBeanInfo(). Expected results:
   * <UL>
   * <LI>BeanDescriptor.getBeanClass() will return
   ...
   * is returned from getAdditionalBeanInfo(). <br>
   * Expected results:
   * <ul>
   * <li>0 will be returned.
   * </ul>
  ...
  /**
   * Verify AS400JDBCConnectionPoolDataSourceBeanInfo::getEventSetDescriptors() is
   * returned from getAdditionalBeanInfo(). <br>
   * Expected results:
   * <ul>
   ...
   * <li>The appropriate EventDescriptor.getListenerMethods() will be returned for
   * each EventDescriptor.
   * <li>The appropriate EventDescriptor.getRemoveListenerMethod() will be
   * returned for each EventDescriptor.
  ...
  public void Var007() {
    try {
      ...
      if (!ed[0].getRemoveListenerMethod().getName().equals("removePropertyChangeListener")) {
        failed("Wrong Remove listener method returned: " + ed[0].getRemoveListenerMethod().getName());
        return;
      }
  ...
  /**
   * Verify AS400JDBCConnectionPoolDataSourceBeanInfo::getXIcon(int) is returned
   * from getAdditionalBeanInfo(). <br>
   * Expected results:
   * <ul>
  ...
  /**
   * Verify AS400JDBCConnectionPoolDataSourceBeanInfo::getPropertyDescriptors() is
   * returned from getAdditionalBeanInfo(). <br>
   * Expected results:
   * <ul>
   * <li>An array of size 46 will be returned.
   * <li>The PropertyDescriptor.getPropertyEditorClass() will return correct
   * information::
  ...
  public void Var009() {
    ...
    try {
      AS400JDBCConnectionPoolDataSourceBeanInfo bi = new AS400JDBCConnectionPoolDataSourceBeanInfo();
      BeanInfo[] bis = bi.getAdditionalBeanInfo();
      int expectedLength = 121; /* trim char fields + authenticationVerificationId */
      PropertyDescriptor[] pd = bis[0].getPropertyDescriptors();
      if (pd.length != expectedLength) {
        failMessage.append("Wrong number of property descriptors returned: " + String.valueOf(pd.length) + " expected "
            + expectedLength + "\n");
        passed = false;
      }
      ...
      for (int i = 0; i < pd.length; i++) {
        if (pd[i].getName().equals("password")) // password.
        {
          ...
          if (!pd[i].getReadMethod().getName().equals(value)) {
            ...
            if (name.equals("toolboxTrace") && pd[i].getReadMethod().getName().equals("getToolboxTrace")) {
              // OK Skip
            } else if (name.equals("serverTrace")
                && pd[i].getReadMethod().getName().equals("getServerTraceCategories")) {
            } else {

              if (pd[i].getReadMethod().getName().equals("isTcpNoDelay") && value.equals("getTcpNoDelay")) {
                // Ignore this mismatch
              } else {
                failMessage.append("Wrong read method for " + pd[i].getName() + ": " + pd[i].getReadMethod().getName()
                    + " not " + value + "\n");
                passed = false;
              }
            }
      ...
      for (int i = 0; i < pd.length; i++) {
        ...
        if (!pd[i].getWriteMethod().getName().equals(value)) {
          ...
          if (!pd[i].getWriteMethod().getName().equals(value2)) {
            ...
            failMessage.append("Wrong write method for " + pd[i].getName() + ": " + pd[i].getWriteMethod().getName()
                + " expected " + value + "or " + value2 + "\n");
            passed = false;
          }
        }
      }
      ...
      propertyShortDescs.put("bigDecimal",
          "Specifies whether an intermediate java.math.BigDecimal object is used for packed and zoned decimal conversions.");
      propertyShortDescs.put("blockCriteria",
          "Specifies the criteria for retrieving data from the system in blocks of records."); // @A1C //@D2C
      propertyShortDescs.put("blockSize",
          "Specifies the block size (in kilobytes) to retrieve from the system and cache on the client.");
      ...
      propertyShortDescs.put("dateSeparator",
          "Specifies the date separator used in date literals within SQL statements.");
      propertyShortDescs.put("decimalSeparator",
          "Specifies the decimal separator used in numeric constants within SQL statements.");
      ...
      propertyShortDescs.put("extendedMetaData", "Specifies whether to request extended metadata from the system."); // @A3A
                                                                                                                     // @D2C
      ...
      propertyShortDescs.put("lobThreshold",
      ...
      propertyShortDescs.put("prompt",
          "Specifies whether the user should be prompted if a user name or password is needed to connect to the system."); // @A1C
                                                                                                                           // @D2C
      propertyShortDescs.put("proxyServer",
          "Specifies the host name and (optionally) port number of the middle-tier machine where the proxy server is running.");
      propertyShortDescs.put("remarks",
          "Specifies the source of the text for REMARKS columns in ResultSet objects returned by DatabaseMetaData methods.");
      propertyShortDescs.put("savePasswordWhenSerialized",
          "Specifies whether to save the password when the data source object is serialized."); // @A3A
      propertyShortDescs.put("secondaryUrl",
          "Specifies the secondary URL to be used for a connection on the middle-tier's DriverManager in a multiple tier environment.");
      propertyShortDescs.put("secondaryURL",
          "Specifies the secondary URL to be used for a connection on the middle-tier's DriverManager in a multiple tier environment.");
      propertyShortDescs.put("secure",
          "Specifies whether a Secure Sockets Layer (SSL) connection is used to communicate with the system."); // @D2C
      ...
      propertyShortDescs.put("sortLanguage",
          "Specifies a 3-character language ID to use for selection of a sort sequence.");
      propertyShortDescs.put("sortTable",
          "Specifies the library and file name of a sort sequence table stored on the system."); // @A1C @D2C
      ...
      propertyShortDescs.put("timeSeparator",
      ...
      propertyShortDescs.put("keepAlive",
          "Specifies the socket keep alive value to use when connecting to the system."); // @D2C
      propertyShortDescs.put("receiveBufferSize",
          "Specifies the socket receive buffer size to use when connecting to the system."); // @D2C
      propertyShortDescs.put("sendBufferSize",
          "Specifies the socket send buffer size to use when connecting to the system."); // @D2C
      ...
      propertyShortDescs.put("tcpNoDelay",
          "Specifies the socket TCP no delay value to use when connecting to the system."); // @D2C
      propertyShortDescs.put("packageCCSID",
          "Specifies the character encoding to use for the SQL package and any statements sent to the system.");// @D2C
      propertyShortDescs.put("packageCcsid",
          "Specifies the character encoding to use for the SQL package and any statements sent to the system."); // @D2C
      propertyShortDescs.put("minimumDivideScale",
      ...
      propertyShortDescs.put("loginTimeout",
          "Specifies the maximum time in seconds that this data source can wait while attempting to connect to a database.");
      propertyShortDescs.put("trueAutoCommit", "Specifies whether the connection should use true auto commit support.");
      propertyShortDescs.put("trueAutocommit", "Specifies whether the connection should use true auto commit support.");
      propertyShortDescs.put("holdInputLocators",
          "Specifies if input locators should be of type \"hold\" or \"no hold\".");
      propertyShortDescs.put("bidiImplicitReordering", "Specifies if bidi implicit LTR-RTL reordering should be used.");
      propertyShortDescs.put("bidiNumericOrdering",
          "Specifies if the numeric ordering round trip feature should be used.");
      propertyShortDescs.put("holdStatements",
          "Specifies if statements should remain open until a transaction boundary.");
      propertyShortDescs.put("rollbackCursorHold", "Specifies whether to hold cursors across a rollback."); // @C1A
      propertyShortDescs.put("variableFieldCompression",
          "Specifies whether variable-length fields should be compressed."); // @C1A
      propertyShortDescs.put("queryOptimizeGoal",
          "Specifies the goal the system should use with optimization of queries."); // @C1A @D2C
      propertyShortDescs.put("xaLooselyCoupledSupport",
          "Specifies whether lock sharing is allowed for loosely coupled transaction branches."); // @PDA
      propertyShortDescs.put("translateBoolean",
          "Specifies how Boolean objects are interpreted when setting the value for a character field/parameter."); // @PDA
      propertyShortDescs.put("metaDataSource", "Specifies how to retrieve DatabaseMetaData."); // @D1A
      propertyShortDescs.put("metadataSource", "Specifies how to retrieve DatabaseMetaData."); // @D1A
      propertyShortDescs.put("queryStorageLimit",
          "Specifies the query storage limit to be used when statements in a connection are executed."); // @pda
      propertyShortDescs.put("decfloatRoundingMode",
          "Specifies the rounding mode to use when working with decfloat data type."); // @pda
      propertyShortDescs.put("autocommitException",
          "Specifies whether to throw an SQLException when Connection.commit() or Connection.rollback() is called if autocommit is enabled."); // @D3A
      propertyShortDescs.put("autoCommit",
          "Specifies whether auto-commit mode is the default connection mode for new connections."); // @AC1
      propertyShortDescs.put("ignoreWarnings",
          "Specifies a list of SQL states for which the driver should not create warning objects.");
      propertyShortDescs.put("secureCurrentUser",
          "Specifies whether to disallow \"\" and *current as user name and password.");
      propertyShortDescs.put("concurrentAccessResolution",
          "Specifies whether \"currently committed\" access is used on the connection."); // @cc1
      ...
      propertyShortDescs.put("maximumBlockedInputRows",
          "Specifies the maximum number of rows to be sent to the database engine when using a blocked insert or update operation.");

      propertyShortDescs.put("queryTimeoutMechanism", "Specifies the method used to implement the query timeout.");

      propertyShortDescs.put("queryReplaceTruncatedParameter",
          "Specifies the string value to be used when a query parameter is truncated.");
      ...
      propertyShortDescs.put("useBlockUpdate",
          "Specifies the use of a block update mode when inserting or updating blocks of data into the database.");
      ...
      propertyShortDescs.put("useDrdaMetadataVersion",
          "Specifies if the DRDA metadata version information should be returned.");

      propertyShortDescs.put("enableClientAffinitiesList", "Specifies if alternate servers should be used.");
      propertyShortDescs.put("affinityFailbackInterval",
          "Specifies the length, in seconds, of the interval after which the primary connection will be re-established.");
      ...
      propertyShortDescs.put("maxRetriesForClientReroute",
          "The maximum number of connection retries for automatic client reroute.");
      propertyShortDescs.put("retryIntervalForClientReroute",
          "The number of seconds between consecutive connection retries.");
      propertyShortDescs.put("enableSeamlessFailover",
          "Specifies whether the JTOpen JDBC driver uses seamless failover for client reroute.");

      propertyShortDescs.put("extendedMetadata", "extendedMetadata");
      propertyShortDescs.put("portNumber", "Specifies the port number used to connect to the ZDA server.");
      propertyShortDescs.put("additionalAuthenticationFactor",
          "Specifies the additional authentication factor to be used in conjunction with the password.");
      propertyShortDescs.put("stayAlive",
          "Specifies the number of seconds between pings to the Host Server.  This is used to prevent a connection from being viewed as inactive.  A value of 0 means to not send pings to keep the connection alive.");
      ...
      for (int i = 0; i < pd.length; i++) {
        ...
        if (!verifyFeatureDescriptor(pd[i], pd[i].getDisplayName(),
            (String) propertyShortDescs.get(pd[i].getDisplayName()), false, false, failMessage))
          passed = false;
      }
